...
def test_pywsgi_request_timeout_application(target_application):
    for i in range(3):
        target_application.get("/request-timeout-application/pywsgi", status=500)
...
def test_pywsgi_request_timeout_response(target_application):
    ...
    for i in range(3):
        target_application.get("/request-timeout-response/pywsgi", status=500)
...
def test_pywsgi_request_timeout_finalize(target_application):
    ...
    for i in range(3):
        target_application.get("/request-timeout-finalize/pywsgi", status=500)
        # response.mustcontain('WSGI RESPONSE')


def test_pywsgi_raise_exception_application(target_application):
    for i in range(3):
        target_application.get("/raise-exception-application/pywsgi", status=500)


def test_pywsgi_raise_exception_response(target_application):
    for i in range(3):
        target_application.get("/raise-exception-response/pywsgi", status=500)


def test_pywsgi_raise_exception_finalize(target_application):
    for i in range(3):
        target_application.get("/raise-exception-finalize/pywsgi", status=500)
