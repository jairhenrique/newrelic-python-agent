...
_scoped_metrics = [
    ...
    (
        f"Function/django.contrib.sessions.middleware:SessionMiddleware{process_request_method}",
        1,
    ),
    (
        f"Function/django.contrib.auth.middleware:AuthenticationMiddleware{process_request_method}",
        1,
    ),
    (
        f"Function/django.contrib.messages.middleware:MessageMiddleware{process_request_method}",
        1,
    ),
    ...
    (
        f"Function/django.contrib.messages.middleware:MessageMiddleware{process_response_method}",
        1,
    ),
    (f"Function/django.middleware.csrf:CsrfViewMiddleware{process_response_method}", 1),
    (
        f"Function/django.contrib.sessions.middleware:SessionMiddleware{process_response_method}",
        1,
    ),
...
@validate_transaction_metrics(
    "views:index", scoped_metrics=_test_application_index_scoped_metrics
)
...
@validate_transaction_errors(errors=[])
@validate_transaction_metrics(
    "urls:View.get", scoped_metrics=_test_application_view_scoped_metrics
)
...
@validate_transaction_metrics(
    "urls:ViewError.get", scoped_metrics=_test_application_view_error_scoped_metrics
)
...
def test_application_view_handle_error(
    status, should_record, use_global_exc_handler, target_application
):
    errors = ["urls:Error"] if should_record else []

    @validate_transaction_errors(errors=errors)
    @validate_transaction_metrics(
        "urls:ViewHandleError.get",
        scoped_metrics=_test_application_view_handle_error_scoped_metrics,
    )
    def _test():
        response = target_application.get(
            f"/view_handle_error/{status}/{use_global_exc_handler}/", status=status
        )
...
_test_api_view_scoped_metrics_get.append(
    (f"Function/{_test_api_view_view_name_get}", 1)
)


@validate_transaction_errors(errors=[])
@validate_transaction_metrics(
    _test_api_view_view_name_get, scoped_metrics=_test_api_view_scoped_metrics_get
)
...
_test_api_view_scoped_metrics_post.append(
    (f"Function/{_test_api_view_view_name_post}", 1)
)


@validate_transaction_errors(errors=["rest_framework.exceptions:MethodNotAllowed"])
@validate_transaction_metrics(
    _test_api_view_view_name_post, scoped_metrics=_test_api_view_scoped_metrics_post
)
