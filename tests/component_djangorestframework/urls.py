...
class ViewHandleError(APIView):
    settings = APISettings(
        api_settings.user_settings, api_settings.defaults, api_settings.import_strings
    )
    ...
    def handle_exception(self, exc):
        ...
        if self.global_exc:
            ...
            return Response(
                [{"response": "exception was handled not global"}], status=self.status
            )
...
urlpatterns = [
    ...
    url(
        r"^view_handle_error/(?P<status>\d+)/(?P<global_exc>\w+)/$",
