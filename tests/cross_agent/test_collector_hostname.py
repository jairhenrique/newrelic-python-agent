...
FIXTURE = os.path.normpath(
    os.path.join(CURRENT_DIR, "fixtures", "collector_hostname.json")
)

_parameters_list = [
    "config_file_key",
    "config_override_host",
    "env_key",
    "env_override_host",
    "hostname",
]
...
def _test_collector_hostname(
    ...
    config_override_host=None,
    env_key=None,
    env_override_host=None,
    hostname=None,
    queue=None,
):
    try:
...
@pytest.mark.parametrize(
    _parameters, _collector_hostname_tests, ids=_collector_hostname_ids
)
def test_collector_hostname(
    config_file_key, config_override_host, env_key, env_override_host, hostname
):
