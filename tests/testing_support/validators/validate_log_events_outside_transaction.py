...
def validate_log_events_outside_transaction(
    events=None, required_attrs=None, forgone_attrs=None
):
    events = (
        events or [{}]
    )  # Empty event allows assertions based on only forgone attrs to still run and validate
    ...
    def _validate_wrapper(wrapped, instance, args, kwargs):
        ...
        @transient_function_wrapper(
            "newrelic.core.stats_engine", "StatsEngine.record_log_event"
        )
        ...
        for expected in events:
            ...
            for captured in logs:
                if _check_log_attributes(
                    expected, required_attrs, forgone_attrs, captured, mismatches
                ):
                    matching_log_events += 1
            assert matching_log_events == 1, _log_details(
                matching_log_events, logs, mismatches
            )

        return val

    def _check_log_attributes(
        expected, required_attrs, forgone_attrs, captured, mismatches
    ):
        for key, value in expected.items():
            if hasattr(captured, key):
                ...
                mismatches.append(
                    f"key: {key}, value:<{value}><{getattr(captured, key, None)}>"
                )
                return False
